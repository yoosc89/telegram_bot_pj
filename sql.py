...
def select_user_info(telegram_id):
    cursor.execute("select * from user_info where telegram_id = %s;",telegram_id)
...
def create_user(user_data):
    cursor.execute('insert into user_info(telegram_id, site_list, alert_site_index) values(%s, %s, %s, %s);', user_data)

def insert_user_info(telegram_id, user_site):
    cursor.execute("insert into telegram_bot.user_info (telegram_id, site_list) values (%s,%s);",[telegram_id, [user_site]])
    conn.commit()

def update_user_info_site_index(telegram_id, index):
    cursor.execute("update user_info set alert_site_index = %s where telegram_id = %s;",[index, telegram_id])
    conn.commit()

def update_user_info_site_list(telegram_id, site_list):
    cursor.execute("update user_info set site_list = %s where telegram_id = %s;",[site_list, telegram_id])
    conn.commit()
...
def insert_alert_site(data):
    for i in data:
        try:
            cursor.executemany("insert into telegram_bot.alert_site (site_name, site_title, site_url, site_domain) values (%s,%s,%s,%s);",[i])
