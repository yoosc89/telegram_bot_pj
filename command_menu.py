...
def site_del(update, context):
    type_code = '02'
    currunt_Slist = list(db_loads.serach_user_info(update.message.chat.id))[2].split(',')
    temp_list = [] 
    buttons = []
    ...
    buttons = []
    for i in temp_list:
        buttons.append([InlineKeyboardButton(i[2], callback_data=type_code+i[0])])
    
    reply_markup = InlineKeyboardMarkup(buttons)
    if buttons != []:
        context.bot.send_message(update.message.chat.id,'삭제할 사이트를 선택하세요',reply_markup=reply_markup)
    else :
...
def site_add(update, context):    
    ...
    context.bot.send_message(update.message.chat.id,'등록할 사이트를 선택하세요',reply_markup=reply_markup)


def callback(update, context):
    callback_code = update.callback_query.data
    site_name = site_code(callback_code[2:], 'code')[1]
    click = db_loads.type_code(callback_code)(update.effective_user.id, site_name)
